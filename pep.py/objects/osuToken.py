...
class token:
	def __init__(self, userID, token_ = None, ip ="", irc = False, timeOffset = 0, tournament = False):
		...
		if userID == 1000:
			...
			self.actionID = actions.UNKNOWN

		if userID == 1000:
			self.actionText = "When You See It"
		else:
			self.actionText = "\nThis guy is don't touching osu"
		self.actionMd5 = ""
		self.actionMods = 0
		...
		# Stats cache2
		if userID in (1001, 1014, 1165):
			self.actionID = actions.WATCHING
			self.actionText = "Debian \nWYSI"

		# Stellar, UNION!!
		if userID in (1124, 1184):
			self.actionID = actions.LOBBY
			self.actionText = "This guy is Special"
		
		if userID == 8016:
			self.actionID = actions.WATCHING
			self.actionText = "This guy UserID is 8016!! \n8016 was owner's osu!Debian UserID"
		
		# Relax
		self.relaxing = False
		self.autopiloting = False
		self.relaxAnnounce = False
		self.autopilotAnnounce = False
	...
	def updateCachedStats(self):
		...
		stats_autopilot = userUtils.getUserStatsAp(self.userID, self.gameMode)
		log.debug(str(stats))
		...
		if self.relaxing:
			...
			self.gameRank = stats_autopilot["gameRank"]
			self.pp = stats_autopilot["pp"]
			self.rankedScore = stats_autopilot["rankedScore"]
			self.accuracy = stats_autopilot["accuracy"]/100
			self.playcount = stats_autopilot["playcount"]
			self.totalScore = stats_autopilot["totalScore"]
		else:
	...
	def setRestricted(self):
		...
		chat.sendMessage(glob.BOT_NAME, self.username, "Your account is currently in restricted mode. Please visit Redstar's website for more information.")
