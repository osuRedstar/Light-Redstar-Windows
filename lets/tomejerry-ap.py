...
class LwScore:
    ...
    def __init__(self, score_id: Optional[int]=None, pp: Optional[int]=None, score_: Optional[scoreAutopilot.score]=None):
        """
...
class Worker:
    ...
    def recalc_score(self, score_data: Dict) -> scoreAutopilot:
        """
        ...
        s: scoreAutopilot.score = scoreAutopilot.score()
...
def mass_recalc(recalculator: Recalculator, workers_number: int=MAX_WORKERS, chunk_size: Optional[int]=None):
    ...
    LOVED_SCORES_LOGGER.addHandler(
        logging.FileHandler("tomejerry-ap_loved_scores_{}.log".format(time.strftime("%d-%m-%Y--%H-%M-%S")))
    )
...
def main():
    ...
    recalculators_gen = {
        ...
        "fixstdhd": lambda: SimpleRecalculator(("scoresapx.completed = 3", "scores_ap.play_mode = 0", "scores_ap.mods & 8 > 0"))
    }
    recalculator = None
