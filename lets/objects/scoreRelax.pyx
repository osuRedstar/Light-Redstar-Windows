...
class score:
	...
	def setDataFromDB(self, scoreID, rank = None):
		...
		data = glob.db.fetch("SELECT scores_relax.*, users.username, users.ban_datetime FROM scores_relax LEFT JOIN users ON users.id = scores_relax.userid WHERE scores_relax.id = %s LIMIT 1", [scoreID])
		if data is not None:
		...
		try:
			...
			if self.passed and scoreUtils.isRankable(self.mods):
				...
				if b.rankedStatus == rankedStatuses.LOVED and False:
					#personalBest = glob.db.fetch("SELECT id, score FROM scores_relax WHERE userid = %s AND beatmap_md5 = %s AND play_mode = %s AND completed = 3 DESC LIMIT 1", [userID, self.fileMd5, self.gameMode])
					personalBest = glob.db.fetch("SELECT id, score FROM scores_relax WHERE userid = %s AND beatmap_md5 = %s AND play_mode = %s AND completed = 3 ORDER BY score DESC LIMIT 1", [userID, self.fileMd5, self.gameMode])
				else:
					...
					personalBest = glob.db.fetch("SELECT id,{}score FROM scores_relax WHERE userid = %s AND beatmap_md5 = %s AND play_mode = %s AND completed = 3 ORDER BY pp DESC LIMIT 1".format(
						glob.conf.extra["lets"]["submit"]["score-overwrite"] == "score" and " " or " {}, ".format(glob.conf.extra["lets"]["submit"]["score-overwrite"])
					),
					[userID, self.fileMd5, self.gameMode])
