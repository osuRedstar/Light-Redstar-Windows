...
class score:
	...
	def setDataFromDB(self, scoreID, rank = None):
		...
		data = glob.db.fetch("SELECT scores.*, users.username, users.ban_datetime FROM scores LEFT JOIN users ON users.id = scores.userid WHERE scores.id = %s LIMIT 1", [scoreID])
		if data is not None:
