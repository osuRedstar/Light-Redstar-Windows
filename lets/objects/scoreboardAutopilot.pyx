...
class scoreboardAutopilot:
	...
	def __init__(self, username, gameMode, beatmap, setScores = True, country = False, friends = False, mods = -1):
		...
		setScores -- if True, will get personal/top 50 autopilot scores automatically. Optional. Default: True
		"""
		...
		self.mods = mods
		#self.autopilot = 1
		self.autopilot = "ap" #userUtils.PPBoard() 에서 문자로 모드 결정함
		self.ppboard = 1

		#ppboard 2는 모든 랭크상태 통틀어서 ppboard, 
		#ppboard 3는 모든 랭크상태 통틀어서 scoreboard임
		if userUtils.PPBoard(self.userID, self.autopilot) >= 2:
			if userUtils.PPBoard(self.userID, self.autopilot) == 2:
				self.ppboard = 1
				log.chat(f"{self.username} ({self.userID}) Used !ppboard ap force")
			elif userUtils.PPBoard(self.userID, self.autopilot) == 3:
				self.ppboard = 0
				log.chat(f"{self.username} ({self.userID}) Used !scoreboard ap force")
		else:
			if beatmap.rankedStatus == 5 or beatmap.rankedStatus == 4:
				self.ppboard = 0
				if userUtils.PPBoard(self.userID, self.autopilot) == 0:
					self.ppboard = 1
			else:
				self.ppboard = 1
				if userUtils.PPBoard(self.userID, self.autopilot) == 0:
		...
		""" if glob.conf.extra["lets"]["submit"]["loved-dont-give-pp"] and beatmap.rankedStatus == 5:
			...
			self.ppboard = 1
		elif userUtils.PPBoard(self.userID, self.autopilot) == 1:
			self.ppboard = 1
		else:
			self.ppboard = 0 """
		if setScores:
	...
	def setScores(self):
		...
		if personalBestScore is not None:
			s = scoreAutopilot.score(personalBestScore)
			self.scores[0] = s
		else:
			# No personal best
			self.scores[0] = -1
		...
		if priv:
			joins = "FROM scores_ap STRAIGHT_JOIN users ON scores_ap.userid = users.id STRAIGHT_JOIN users_stats ON users.id = users_stats.id WHERE scores_ap.beatmap_md5 = %(beatmap_md5)s AND scores_ap.play_mode = %(play_mode)s AND scores_ap.completed = 3 AND (1 OR users.id = %(userid)s)"
		else:
			joins = "FROM scores_ap STRAIGHT_JOIN users ON scores_ap.userid = users.id STRAIGHT_JOIN users_stats ON users.id = users_stats.id WHERE scores_ap.beatmap_md5 = %(beatmap_md5)s AND scores_ap.play_mode = %(play_mode)s AND scores_ap.completed = 3 AND (users.privileges & 1 > 0 OR users.id = %(userid)s)"
		...
		if self.mods > -1 and self.mods & modsEnum.AUTOPLAY == 0:
			...
			joins = joins.replace("scores_ap.completed = 3", "scores_ap.completed != 0")
		else:
			mods = ""
		...
		try:
			...
			if self.beatmap.fileMD5 == "e8983455534ebb6dbc27740943b4e436":
				for i in topScores:
					log.debug(f"Autopilot | [https://{server_domain}/u/{i['userid']} {i['username']}] | scoreAutopilot's id = {i['id']} | beatmap_md5 = {i['beatmap_md5']} | pp = {i['pp']} --> 1000002 | score = {i['score']} --> 1000000")
					i["pp"] = 1000002
					i["score"] = 1000000
		except:
		...
		if topScores is not None:
			for topScore in topScores:
				# Create score object
				s = scoreAutopilot.score(topScore["id"], setData=False)
