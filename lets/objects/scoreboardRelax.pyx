...
class scoreboardRelax:
	...
	def __init__(self, username, gameMode, beatmap, setScores = True, country = False, friends = False, mods = -1):
		...
		self.mods = mods
		#self.relax = 1
		self.relax = "rx" #userUtils.PPBoard() 에서 문자로 모드 결정함
		self.ppboard = 1

		#ppboard 2는 모든 랭크상태 통틀어서 ppboard, 
		#ppboard 3는 모든 랭크상태 통틀어서 scoreboard임
		if userUtils.PPBoard(self.userID, self.relax) >= 2:
			if userUtils.PPBoard(self.userID, self.relax) == 2:
				self.ppboard = 1
				log.chat(f"{self.username} ({self.userID}) Used !ppboard rx force")
			elif userUtils.PPBoard(self.userID, self.relax) == 3:
				self.ppboard = 0
				log.chat(f"{self.username} ({self.userID}) Used !scoreboard rx force")
		else:
			if beatmap.rankedStatus == 5 or beatmap.rankedStatus == 4:
				self.ppboard = 0
				if userUtils.PPBoard(self.userID, self.relax) == 0:
					self.ppboard = 1
			else:
				self.ppboard = 1
				if userUtils.PPBoard(self.userID, self.relax) == 0:
	...
	def setScores(self):
		...
		if priv:
			joins = "FROM scores_relax STRAIGHT_JOIN users ON scores_relax.userid = users.id STRAIGHT_JOIN users_stats ON users.id = users_stats.id WHERE scores_relax.beatmap_md5 = %(beatmap_md5)s AND scores_relax.play_mode = %(play_mode)s AND scores_relax.completed = 3 AND (1 OR users.id = %(userid)s)"
		else:
			joins = "FROM scores_relax STRAIGHT_JOIN users ON scores_relax.userid = users.id STRAIGHT_JOIN users_stats ON users.id = users_stats.id WHERE scores_relax.beatmap_md5 = %(beatmap_md5)s AND scores_relax.play_mode = %(play_mode)s AND scores_relax.completed = 3 AND (users.privileges & 1 > 0 OR users.id = %(userid)s)"
		...
		if self.mods > -1 and self.mods & modsEnum.AUTOPLAY == 0:
			...
			joins = joins.replace("scores_relax.completed = 3", "scores_relax.completed != 0")
		else:
			mods = ""
		...
		try:
			...
			if self.beatmap.fileMD5 == "e8983455534ebb6dbc27740943b4e436":
				for i in topScores:
					log.debug(f"Relax | [https://{server_domain}/u/{i['userid']} {i['username']}] | scoreRelax's id = {i['id']} | beatmap_md5 = {i['beatmap_md5']} | pp = {i['pp']} --> 1000000 | score = {i['score']} --> 1000001")
					i["pp"] = 1000000
					i["score"] = 1000001
		except:
