...
class userStatsCacheAP:
	def get(self, userID, gameMode):
		...
		"""
		data = glob.redis.get("lets:user_stats_cache_ap:{}:{}".format(gameMode, userID))
		if data is None:
	...
	def update(self, userID, gameMode, data = None):
		...
		glob.redis.set("lets:user_stats_cache_ap:{}:{}".format(gameMode, userID), json.dumps(data), 1800)