...
class handler(requestsManager.asyncRequestHandler):
	...
	def asyncGet(self):
		try:
			...
			isCustomBeatmap = glob.db.fetch("SELECT beatmap_id FROM beatmaps WHERE beatmap_md5 = %s AND beatmap_id < 0", [md5])
			if isCustomBeatmap:
				#userToken.enqueue(serverPackets.notification("You're playing with Relax, we've changed the leaderboard to Relax."))
				import requests
				try:
					r = requests.get(f"{glob.conf.config['server']['banchourl']}/api/v1/fokabotMessage?k={glob.conf.config['server']['apikey']}&to={username}&msg=This Map Is Custom Beatmap!! [osu://b/{isCustomBeatmap['beatmap_id']} {fileName}]", headers={"User-Agent": "lets.py"})
				except exceptions as e:
					log.error(f"커스텀 비트맵 감지 코드 에러 | {e}")
				#pass
			...
			if bool(mods & 128):
					...
					sboard = scoreboardAutopilot.scoreboardAutopilot(
					username, gameMode, bmap, setScores=True, country=country, mods=modsFilter, friends=friends
					)
			else:
