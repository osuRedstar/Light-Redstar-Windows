...
class oppai:
	...
	def __init__(self, __beatmap, __score = None, acc = 0, mods = 0, tillerino = False):
		...
		# Calculate pp
		log.debug("oppai-autopilot ~> Initialized oppai diffcalc")
		self.calculatePP()

	@staticmethod
	def _runOppaiProcess(command):
		#log.debug("oppai-autopilot ~> running {}".format(command))
		log.chat("oppai-autopilot ~> running {}".format(command))
		...
		try:
			...
			if output["code"] != 200:
				raise OppaiError("oppai-autopilot error {}: {}".format(output["code"], output["errstr"]))
			if "pp" not in output or "stars" not in output:
				raise OppaiError("No pp/stars entry in oppai-autopilot json output")
			pp = output["pp"]
			stars = output["stars"]

			log.debug("oppai-autopilot ~> full output: {}".format(output))
			log.debug("oppai-autopilot ~> pp: {}, stars: {}".format(pp, stars))
	...
	def calculatePP(self):
		...
		try:
			...
			#log.debug("oppai-autopilot ~> Map file: {}".format(mapFile))
			log.warning("oppai ~> Map file: {}".format(mapFile))
			...
			command = "B:/redstar/lets/pp/oppai-ng/oppai {}".format(mapFile)
			...
			log.debug("oppai-autopilot ~> Calculated PP: {}, stars: {}".format(self.pp, self.stars))
		except OppaiError:
			log.error("oppai-autopilot ~> oppai-ng error!")
			self.pp = 0
		except exceptions.osuApiFailException:
			log.error("oppai-autopilot ~> osu!api error!")
			self.pp = 0
		except exceptions.unsupportedGameModeException:
			log.error("oppai-autopilot ~> Unsupported gamemode")
			self.pp = 0
		except Exception as e:
			log.error("oppai-autopilot ~> Unhandled exception: {}".format(str(e)))
			self.pp = 0
			raise
		finally:
			log.debug("oppai-autopilot ~> Shutting down, pp = {}".format(self.pp))
